...
void draw_raw_cgo(const NA_Matrix &list_of_pockets, const Poly_Vector &polys,
                  std::string &out_script_template,
...
void draw_raw_polyhedrons(std::ofstream &pymol_script,
                          const Poly_Vector &CH_vec,
                          const std::string &model_template) {
  ...
  for (const auto &CH : CH_vec) {
    ...
    for (fc_ite = CH.facets_begin(); fc_ite != fc_ite_end; ++fc_ite) {
      P_Halfedge_around_facet_const_circulator he_circ = fc_ite->facet_begin();
...
void draw_grid_cgo(const NA_Matrix &list_of_pockets,
  ...
  if (precision == 1) {
    ...
    pymol_script << "\tCOLOR,  " << red[cont % col_nbr] << ",\t"
                 << green[cont % col_nbr] << ",\t" << blue[cont % col_nbr]
                 << "," << '\n';
    draw_grid_cgo_polyhedrons(pymol_script, in_vtces_radii, intersecting_total,
                              polys, sphere_size, sphere_count);
    pymol_script << "\t]\ncmd.load_cgo(" << model_name << ", '" << model_name
                 << "',   1)" << '\n';
  }
  return;
    ...
    for (unsigned int i = 0; i < sphere_count; i++) {
      for (unsigned int j = 0; j <= sphere_count - i; j++) {
        for (unsigned int k = 0; k <= sphere_count - i - j; k++) {
          ...
          point[0] = (i * p0_[0] + j * p1_[0] + k * p2_[0] + l * p3_[0]) / n;
          point[1] = (i * p0_[1] + j * p1_[1] + k * p2_[1] + l * p3_[1]) / n;
          point[2] = (i * p0_[2] + j * p1_[2] + k * p2_[2] + l * p3_[2]) / n;

          if (i <= corner || j <= corner || k <= corner) {
            // Check if any sphere is in conflict with a vtx
            const double dist0 = std::sqrt(pow((point[0] - p0_[0]), 2.0) +
                                           pow((point[1] - p0_[1]), 2.0) +
                                           pow((point[2] - p0_[2]), 2.0));
            if (dist0 < VdW_0 + sphere_size) {
              continue;
            }
            const double dist1 = std::sqrt(pow((point[0] - p1_[0]), 2.0) +
                                           pow((point[1] - p1_[1]), 2.0) +
                                           pow((point[2] - p1_[2]), 2.0));
            if (dist1 < VdW_1 + sphere_size) {
              continue;
            }
            const double dist2 = std::sqrt(pow((point[0] - p2_[0]), 2.0) +
                                           pow((point[1] - p2_[1]), 2.0) +
                                           pow((point[2] - p2_[2]), 2.0));
            if (dist2 < VdW_2 + sphere_size) {
              continue;
            }
            const double dist3 = std::sqrt(pow((point[0] - p3_[0]), 2.0) +
          ...
          pymol_script << "# new cell" << '\n';
          pymol_script << "\tSPHERE,\t" << point[0] << ",\t" << point[1]
                       << ",\t" << point[2] << ",\t" << sphere_size << ","
                       << '\n';
        }
      }
...
void draw_grid_cgo_polyhedrons(
    std::ofstream &pymol_script,
    const std::vector<std::array<double, 3>> &in_vtces_radii,
    const std::vector<unsigned int> &intersecting_total,
  ...
  for (unsigned int ii = 0; ii < CH_vec.size(); ++ii) {
    ...
    case 3: {
      // This polyhedron is a tetrahedron from the intersection of a cell with 1
      // vtx inside the included area
      std::array<double, 3> p0_ = {CGAL::to_double(points[0].x()),
                                   CGAL::to_double(points[0].y()),
                                   CGAL::to_double(points[0].z())};
      std::array<double, 3> p1_ = {CGAL::to_double(points[1].x()),
                                   CGAL::to_double(points[1].y()),
                                   CGAL::to_double(points[1].z())};
      std::array<double, 3> p2_ = {CGAL::to_double(points[2].x()),
                                   CGAL::to_double(points[2].y()),
                                   CGAL::to_double(points[2].z())};
      std::array<double, 3> p3_ = {CGAL::to_double(points[3].x()),
                                   CGAL::to_double(points[3].y()),
                                   CGAL::to_double(points[3].z())};
      for (unsigned int i = 0; i < sphere_count; i++) {
        for (unsigned int j = 0; j <= sphere_count - i; j++) {
          for (unsigned int k = 0; k <= sphere_count - i - j; k++) {
            std::array<double, 3> point;
    ...
    case 2: {
      // This polyhedron comprises 3 tetrahedrons from the intersection of a
      // cell with 2 vtces inside the included area
      for (unsigned int jj = 0; jj < 3; ++jj) {
        std::array<double, 3> p0_ = {CGAL::to_double(points[(jj * 4) + 0].x()),
                                     CGAL::to_double(points[(jj * 4) + 0].y()),
                                     CGAL::to_double(points[(jj * 4) + 0].z())};
        std::array<double, 3> p1_ = {CGAL::to_double(points[(jj * 4) + 1].x()),
                                     CGAL::to_double(points[(jj * 4) + 1].y()),
                                     CGAL::to_double(points[(jj * 4) + 1].z())};
        std::array<double, 3> p2_ = {CGAL::to_double(points[(jj * 4) + 2].x()),
                                     CGAL::to_double(points[(jj * 4) + 2].y()),
                                     CGAL::to_double(points[(jj * 4) + 2].z())};
        std::array<double, 3> p3_ = {CGAL::to_double(points[(jj * 4) + 3].x()),
            ...
            for (unsigned int k = 0; k <= sphere_count - i - j; k++) {
              ...
              if (i <= corner || j <= corner || k <= corner) {
                ...
                case 0: {
                  ...
                  const double dist1 = std::sqrt(pow((point[0] - p3_[0]), 2.0) +
                                                 pow((point[1] - p3_[1]), 2.0) +
                                                 pow((point[2] - p3_[2]), 2.0));
                  if (dist1 < in_vtces_radii[ii][1] + sphere_size) {
                    continue;
                  }
                  break;
                }
                case 1: {
                  ...
                  const double dist0 = std::sqrt(pow((point[0] - p0_[0]), 2.0) +
                                                 pow((point[1] - p0_[1]), 2.0) +
                                                 pow((point[2] - p0_[2]), 2.0));
                  if (dist0 < in_vtces_radii[ii][1] + sphere_size) {
                    continue;
                  }
                }
                case 2: {
          ...
          for (unsigned int j = 0; j <= sphere_count - i; j++) {
            for (unsigned int k = 0; k <= sphere_count - i - j; k++) {
              ...
              if (i <= corner || j <= corner || k <= corner) {
                ...
                switch (jj) {
                case 0: {
                  // 1st, 2nd and 3rd vtces correspond to atoms
                  const double dist0 = std::sqrt(pow((point[0] - p0_[0]), 2.0) +
                                                 pow((point[1] - p0_[1]), 2.0) +
                                                 pow((point[2] - p0_[2]), 2.0));
                  if (dist0 < in_vtces_radii[ii][0] + sphere_size) {
                  ...
                  const double dist2 = std::sqrt(pow((point[0] - p2_[0]), 2.0) +
                                                 pow((point[1] - p2_[1]), 2.0) +
                                                 pow((point[2] - p2_[2]), 2.0));
                  if (dist2 < in_vtces_radii[ii][2] + sphere_size) {
                    continue;
                  }
                  break;
                }
                case 1: {
                  ...
                  const double dist1 = std::sqrt(pow((point[0] - p1_[0]), 2.0) +
                                                 pow((point[1] - p1_[1]), 2.0) +
                                                 pow((point[2] - p1_[2]), 2.0));
                  if (dist1 < in_vtces_radii[ii][2] + sphere_size) {
                    continue;
                  }
                  break;
                }
                case 2: {
                  ...
                  const double dist0 = std::sqrt(pow((point[0] - p0_[0]), 2.0) +
                                                 pow((point[1] - p0_[1]), 2.0) +
                                                 pow((point[2] - p0_[2]), 2.0));
                  if (dist0 < in_vtces_radii[ii][2] + sphere_size) {
                    continue;
                  }
                  break;
                }
  ...
  for (size_t i = 0; i < list_of_pockets.size(); i++) {
    ...
    if (list_of_polys.size() != 0) {
      ...
      for (const auto &polyhedron : list_of_polys[i]) {
        ...
        for (P_Vertex_const_iterator v_ite = v_beg; v_ite != v_end; ++v_ite) {
...
void draw_grid_pdb(
    const NDD_Matrix &list_of_pockets, const Poly_Matrix &list_of_polys,
    const std::vector<std::array<double, 3>> &in_vtces_radii,
  ...
  for (size_t i = 0; i < frame_nbr; i++) {
    ...
    if (precision == 1) {
      atom_cnt_list[i] = make_grid_pdb_polyhedrons(
          ana_void_top, ana_void_frame, in_vtces_radii,
          list_intersecting_total[i], list_of_polys[i], sphere_count,
          atom_cnt_list[i], res_cnt);
    }
  ...
  for (size_t i = 0; i < frame_nbr; i++) {
    ...
    for (size_t j = atom_cnt_list[i]; j < max_atom_cnt; j++) {
      ...
      list_ana_void_frame[i].add_atom(chemfiles::Atom("N"),
                                      chemfiles::Vector3D(0., 0., 0.));
    }
    // Show the extra atoms as a residue.
...
unsigned int make_grid_pdb_polyhedrons(
  ...
  for (unsigned int ii = 0; ii < CH_vec.size(); ++ii) {
    ...
    switch (intersecting_total[ii]) {
    case 3: {
      // This polyhedron is a tetrahedron from the intersection of a cell with
      // 1
      // vtx inside the included area
      std::array<double, 3> p0_ = {CGAL::to_double(points[0].x()),
                                   CGAL::to_double(points[0].y()),
...
void draw_grid_pdb(const NA_Vector &pocket,
  ...
  auto atom_cnt = make_grid_pdb(pocket, ana_void_top, ana_void_frame,
                                sphere_count, res_cnt);

  if (precision == 1) {
    make_grid_pdb_polyhedrons(ana_void_top, ana_void_frame, in_vtces_radii,
                              intersecting_total, polys, sphere_count, atom_cnt,
                              res_cnt);
  }
...
inline void header_script_pymol(std::ofstream &pymol_script,
                                const std::string pdb_filename) {
...
// Write .PDB header
inline void header_PDB(const std::string &out_pdb_filename,
                       const std::string &in_pdb_filename) {
  return;
}

// Construct a residue object with 4 atoms starting at index cell_cnt
inline chemfiles::Residue make_cell_residue(const unsigned int cell_cnt,
                                            const unsigned int atom_cnt) {
...
inline void connect_cell_residue(chemfiles::Topology &topology,
...
inline void connect_facet(chemfiles::Topology &topology,
                          const unsigned int atom_index) {
...
void draw_CH(const Polyhedron &CH, std::string &out_filename) {
  ...
  for (P_Facet_const_iterator fc_ite = CH.facets_begin(); fc_ite != fc_ite_end;
       ++fc_ite) {
    P_Halfedge_around_facet_const_circulator he_circ = fc_ite->facet_begin();
    do {
      ...
      out_frm.add_atom(
          chemfiles::Atom("C"),
          chemfiles::Vector3D(CGAL::to_double(he_circ->vertex()->point().x()),
                              CGAL::to_double(he_circ->vertex()->point().y()),
                              CGAL::to_double(he_circ->vertex()->point().z())));
      ++atom_cnt;
    } while (++he_circ != fc_ite->facet_begin());
...
void get_info_cell(const NA_Vector &null_areas_vtor,
  ...
  for (auto const &cell : null_areas_vtor) {
    for (unsigned int i = 0; i < 3; i++) {
      ...
      a_idx = std::lower_bound(wall_atom_idx.begin(), wall_atom_idx.end(),
                               atom_idx);
...
void get_info_cell(const NA_Vector &null_areas_vtor,
  ...
  for (auto const &cell : null_areas_vtor) {
    for (unsigned int i = 0; i < 3; i++) {
      ...
      // Get position of the new index
      r_idx = std::lower_bound(wall_aa_idx.begin(), wall_aa_idx.end(), res_idx);
