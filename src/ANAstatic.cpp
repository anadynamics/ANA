...
int static_ANA(
  ...
  if (!requested_CH) {
    ...
    if (clusters_method == "facets") {
      ANA::cluster_cells_cgal(cavity_void_cells, null_areas_mtx,
                              clusters_min_size);
  ...
  if (out_filename != "none") {
    if (out_type == "raw_pdb") {
      ...
      for (const NA_Vector &null_areas_vtor : null_areas_mtx) {
        ...
        ANA::draw_raw_PDB(null_areas_vtor, border_poly, pock_out_filename);
        ++pock_cnt;
      }
      ...
      for (const NA_Vector &null_areas_vtor : null_areas_mtx) {
        ...
        pock_out_filename.append(std::to_string(pock_cnt));
        pock_out_filename.append(".pdb");
        ANA::draw_grid_pdb(null_areas_vtor, in_vtces_radii, intersecting_total,
                           border_poly, pock_out_filename, sphere_count,
                           precision);
        ++pock_cnt;
      }
    }
