...
int MD_ANA(const std::string &in_filename, const std::string &in_md_filename,
  ...
  std::vector<std::vector<unsigned int>> list_intersecting_total(nbr_of_frames);
  ...
  while (!in_traj.done()) {
    ...
    if (!requested_CH) {
      ...
      if (precision == 1) {
        ...
        // Store this frame's polyhedrons and the number of intersections.
        polys_md[frame_cnt - 1] = std::move(border_poly);
        list_intersecting_total[frame_cnt - 1] = std::move(intersecting_total);
    ...
    if (list_wall == "atom") {
      ANA::wall_atom_output(wall_out, cavity_void_cells,
                            cavity_intersecting_cells, intersecting_bool,
                            requested_CH, precision, 1, frame_cnt,
                            list_wall_separator);
    ...
    // Get the number of vertices (atoms) to write, look for the highest nbr of
    // "voids_md" them, and store it. Then, store voids for later output.
    unsigned int atom_cnt_step = cavity_void_cells.size() * 4 + atom_cnt_poly;
