...
int main(int argc, char *argv[]) {
  std::string in_filename, AA_indices_proto, exclude_ca_for_ASA_indices_proto,
      ...
      out_filename, clusters_method = "facets", ASA_method = "dot_pdt",
      only_side_ASA, list_wall, list_wall_separator, tool_check_CH,
      tool_pdb_to_ch, sphere_proto, cylinder_proto, prism_proto, tool_pdb_norm,
      tool_aa_to_ca;
  ...
  estado = ANA::get_parameters(
      argc, argv, in_filename, in_md_filename, include_CH_filename,
      include_CH_aa_proto, include_CH_atom_proto, AA_indices_proto,
      triangulate_only_included_aas, atom_only, precision, min_cells_cluster,
      nbr_of_vertices_to_include, md_start, md_step, md_end, minVR, maxSR,
      max_probe, max_probe_length, sphere_size, sphere_count, list_wall,
      list_wall_separator, clusters_method, only_side_ASA, ASA_method,
      exclude_ca_for_ASA_indices_proto, pdbs_list_ndd_filename, out_CH_filename,
      out_ndd_filename, out_filename, out_type, tool_check_CH, tool_pdb_to_ch,
      sphere_proto, cylinder_proto, prism_proto, tool_pdb_norm, tool_aa_to_ca);
  if (estado == 1) {
  ...
  if (tool_check_CH != "none") {
    ...
    if (!requested_CH) {
      std::cerr
          << "No valid input for triangulation of included area. Exiting now."
          << '\n';
      return 0;
    }
    ...
    if (in_md_filename == "none") {
      ...
      while (!in_traj.done()) {
        ...
        ANA::read_MD(in_frame, requested_CH, sphere_proto, cylinder_proto,
                     prism_proto, hetatm_atoms, include_CH_atoms,
  ...
  if (tool_aa_to_ca != "none") {
    ...
    ANA::read_static(in_filename, triangulate_only_included_aas, atom_only,
                     ...
                     cylinder_proto, prism_proto, include_CH_filename,
  ...
  if (in_md_filename == "none") {
    if (pdbs_list_ndd_filename == "none") {
      // Static PDB
      estado = ANA::static_ANA(
          in_filename, AA_indices_proto, ASA_method, only_side_ASA,
          exclude_ca_for_ASA_indices_proto, list_wall, list_wall_separator,
          clusters_method, include_CH_aa_proto, include_CH_atom_proto,
          sphere_proto, cylinder_proto, prism_proto, include_CH_filename,
          out_filename, out_type, triangulate_only_included_aas, atom_only,
          minVR, maxSR, max_probe, max_probe_length, sphere_size, sphere_count,
      ...
      // NDD
      estado = ANA::NDD_ANA(
          in_filename, AA_indices_proto, only_side_ASA, ASA_method,
          exclude_ca_for_ASA_indices_proto, list_wall, list_wall_separator,
          include_CH_aa_proto, include_CH_atom_proto, sphere_proto,
          cylinder_proto, prism_proto, include_CH_filename,
          pdbs_list_ndd_filename, out_ndd_filename, out_filename, out_type,
    ...
    // MD run.
    estado = ANA::MD_ANA(
        in_filename, in_md_filename, AA_indices_proto, ASA_method,
        only_side_ASA, exclude_ca_for_ASA_indices_proto, list_wall,
        list_wall_separator, include_CH_aa_proto, include_CH_atom_proto,
        sphere_proto, cylinder_proto, prism_proto, include_CH_filename,
        out_filename, out_type, triangulate_only_included_aas, atom_only, minVR,
