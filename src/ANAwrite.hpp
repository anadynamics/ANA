...
void ndd_write_out_file(const std::vector<double> &output_volumes,
...
void draw_raw_cgo(const NA_Matrix &list_of_pockets, const Poly_Vector &polys,
                  ...
                  const std::string pdb_filename, const unsigned int precision);
...
void header_script_pymol(std::ofstream &pymol_script,
                         const std::string pdb_filename);
// Write .PDB header
void header_PDB(const std::string &out_pdb_filename,
                const std::string &in_pdb_filename);
// Construct a residue object with 4 atoms starting at index cell_cnt
chemfiles::Residue make_cell_residue(const unsigned int cell_cnt,
...
chemfiles::Residue make_grid_residue(const unsigned int atom_cnt_old,
                                     const unsigned int atom_cnt,
                                     const unsigned int resi);
// Connect a residue object with the atoms starting at index cell_cnt
void connect_cell_residue(chemfiles::Topology &topology,
