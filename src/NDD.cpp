...
int NDD_ANA(std::string const &in_filename, std::string &AA_indices_proto,
    std::string const &only_side_ASA, std::string const &ASA_method,
    ...
    if (out_filename != "none") {
        if (out_type == "raw_pdb") {
            ...
            out_filename.append(".pdb");
            ANA::draw_grid_pdb(null_areas_mtx[0], in_vtces_radii,
                intersecting_total, border_poly, out_filename, sphere_count,
                precision);
        }
    }
