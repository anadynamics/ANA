...
void ndd_nondelaunay_dynamics(const NA_Vector &cavity_void_cells,
  ...
  if (input_pdbs_filename.is_open()) {
    ...
    // They are vectors of vectors of arrays of 4 elements. Each array refers
    ...
    if (precision == 1) {
      while (std::getline(input_pdbs_filename, pdb_filename)) {
        ...
        ANA::NDD::ndd_read_PDB_get_cells(pdb_filename, cells_indices,
                                         include_CH_atoms,
                                         new_cells_coordinates, CH_triangs);
        // Separate fully inside cells and the intersecting ones. Get the volume
        // of this last group.
        ndd_discard_CH_0(new_cells_coordinates, CH_triangs, cavity_void_coords,
                         cavity_intersecting_coords, intersecting_bool,
                         intersecting_total);
        double poly_vol = ndd_discard_CH_1(cavity_intersecting_coords,
                                           CH_triangs, intersecting_bool,
                                           intersecting_total, border_poly);
        // Store result
        output_volumes.push_back(poly_vol +
                                 ndd_get_void_volume(cavity_void_coords));
      }
    } else {
      while (std::getline(input_pdbs_filename, pdb_filename)) {
        NDD_Vector init_cells_coordinates, new_cells_coordinates,
        ...
        // Store result
        output_volumes.push_back(ndd_get_void_volume(new_cells_coordinates));
      }
    }
...
inline double ndd_refine_cell_volume(const double entire_cell_vol,
                                     const NDD_Element &cell) {
...
void ndd_discard_CH_0(const NDD_Vector &in_coords,
  ...
  // Triangle normals point inwards. Only inside points will give a positive dot
    ...
    case 3: {
      for (unsigned int i = 0; i < 4; i++) {
        if (!intersecting_bool[each][i]) {
          ...
          Point p1(
              in_intersecting_coords[each][get_i_not_equal(v, i, 1)].first);
          Point p2(
              in_intersecting_coords[each][get_i_not_equal(v, i, 2)].first);
          Point p3(
              in_intersecting_coords[each][get_i_not_equal(v, i, 3)].first);

          // Get the intersecting segments
          std::vector<Segment> segments = {Segment(p0, p1), Segment(p0, p2),
                                           Segment(p0, p3)};
          ...
          for (unsigned int i = 0; i < i_points.size() - 1; i++) {
            for (unsigned int k = i + 1; k < i_points.size(); k++) {
              if (i_points[i] == i_points[k]) {
                i_points[k] = i_points[k] + Vector(0.01 * i, 0.01 * k, 0.01);
          ...
          Polyhedron CH;
          CH.make_tetrahedron(p0, i_points[0], i_points[1], i_points[2]);
          border_poly.push_back(CH);
          // Add the volume of this polyhedron.
          volume = volume + std::abs(CGAL::to_double(CGAL::volume(
                                p0, i_points[0], i_points[1], i_points[2])));

          // Substract the volume of the sphere sector.
          volume = volume - sphere_sector_vol(p0, i_points[0], i_points[1],
        ...
        if (!intersecting_bool[each][i]) {
          for (unsigned int j = i + 1; j < 4; j++) {
            if (!intersecting_bool[each][j]) {
              ...
              Point p2(
                  in_intersecting_coords[each][get_i_not_equal(v, query_vec, 1)]
                      .first);
              Point p3(
                  in_intersecting_coords[each][get_i_not_equal(v, query_vec, 2)]
                      .first);

              // Get the intersecting segments
              std::vector<Segment> segments = {Segment(p0, p2), Segment(p0, p3),
                                               Segment(p1, p2),
                                               Segment(p1, p3)};
              ...
              for (unsigned int i = 0; i < i_points.size() - 1; i++) {
                for (unsigned int k = i + 1; k < i_points.size(); k++) {
                  if (i_points[k] == i_points[k]) {
                    i_points[k] = i_points[k] + Vector(0.01, 0.01, 0.01);
                    degeneracies_bool = true;
                  }
                }
              }
              ...
              // Construct the polyhedron and get its volume
              Polyhedron CH;
              CH.make_tetrahedron(p0, i_points[0], i_points[1], p1);
              CH.make_tetrahedron(p1, i_points[0], i_points[1], i_points[2]);
              CH.make_tetrahedron(p1, i_points[1], i_points[2], i_points[3]);
              border_poly.push_back(CH);
              // Add the volume of this polyhedron
              volume = volume + std::abs(CGAL::to_double(CGAL::volume(
                                    p0, i_points[0], i_points[1], p1)));
              volume =
                  volume + std::abs(CGAL::to_double(CGAL::volume(
              ...
              volume = volume - sphere_sector_vol(p0, i_points[0], i_points[1],
                                                  p1, VdW_radius_0);
              volume = volume - sphere_sector_vol(p1, i_points[0], i_points[1],
                                                  i_points[2], VdW_radius_1);
              volume = volume - sphere_sector_vol(p1, i_points[1], i_points[2],
            ...
            if (!intersecting_bool[each][j]) {
              for (unsigned int k = j + 1; k < 4; k++) {
                if (!intersecting_bool[each][k]) {
                  ...
                  Point p0(in_intersecting_coords[each][i].first);
                  Point p1(in_intersecting_coords[each][j].first);
                  Point p2(in_intersecting_coords[each][k].first);
                  // p0 p1 p3 are the points inside the included area
                  const double VdW_radius_0 =
                      double(in_intersecting_coords[each][i].second);
                  const double VdW_radius_1 =
                      double(in_intersecting_coords[each][j].second);
                  const double VdW_radius_2 =
                      double(in_intersecting_coords[each][k].second);
                  std::vector<unsigned int> query_vec = {i, j, k};
                  ...
                  std::vector<Segment> segments = {
                      Segment(p0, p3), Segment(p1, p3), Segment(p2, p3)};
                  ...
                  // Construct the polyhedron and get its volume
                  Polyhedron CH;
                  CH.make_tetrahedron(p0, p1, p2, i_points[0]);
                  CH.make_tetrahedron(p0, p2, i_points[0], i_points[1]);
                  CH.make_tetrahedron(p2, i_points[0], i_points[1],
                                      i_points[2]);
                  border_poly.push_back(CH);
                  // Add the volume of this polyhedron
                  volume = volume + std::abs(CGAL::to_double(
                                        CGAL::volume(p0, p1, p2, i_points[0])));
                  volume = volume + std::abs(CGAL::to_double(CGAL::volume(
                                        p0, p2, i_points[0], i_points[1])));
                  volume =
                      volume + std::abs(CGAL::to_double(CGAL::volume(
                                   p2, i_points[0], i_points[1], i_points[2])));

                  // Substract the volume of the sphere sector
                  // 1st tetra
                  volume = volume - sphere_sector_vol(p0, p1, p2, i_points[0],
                                                      VdW_radius_0);
                  volume = volume - sphere_sector_vol(p1, p2, p0, i_points[0],
                                                      VdW_radius_1);
                  volume = volume - sphere_sector_vol(p2, p0, p1, i_points[0],
                                                      VdW_radius_2);
                  // 2nd tetra
                  volume =
                      volume - sphere_sector_vol(p0, p2, i_points[0],
                                                 i_points[1], VdW_radius_0);
                  volume =
                      volume - sphere_sector_vol(p2, p0, i_points[0],
                                                 i_points[1], VdW_radius_2);
                  // 3rd tetra
                  volume =
                      volume - sphere_sector_vol(p2, i_points[0], i_points[1],
...
public:
  ...
  callback_for_boxes(std::vector<unsigned int> &id_vector_a,
                     std::vector<unsigned int> &id_vector_b,
                     NA_Vector &neighbors_a, NA_Vector &neighbors_b)
...
void cluster_cells_boxes(const NA_Vector &input_cells,
                         NA_Matrix &output_cells) {
  ...
  callback_for_boxes called(id_vector_a, id_vector_b, neighbors_a, neighbors_b);
  ...
  // probably bigger than needed, since some cells won't have any intersections
  ...
  cnt = 0;
  ite = std::lower_bound(map_aux.begin(), map_aux.end(),
                         id_vector_b[indices_b[0]]);

  for (const auto &each : indices_b) {
    if (id_vector_b[each] != *ite) {
      // This may be a new ID or just a higher ID that's not were "ite" points,
      // but forward.
      ite = std::lower_bound(map_aux.begin(), map_aux.end(), id_vector_b[each]);
      if (ite == map_aux.end()) {
  ...
  for (size_t i = 0; i < map_aux.size(); i++) {
    ...
    } else if (lb_with_indices(id_vector_b, indices_b, map_aux[i], result) &&
               (id_vector_b[indices_b[result]] == map_aux[i])) {
      neighbors.push_back(neighbors_b[indices_b[result]]);
    } else {
      std::cerr
          << "Iso-oriented boxes clustering method. Invalid starting cell ID."
          << '\n';
      std::exit(1);
    }
    ...
    while (current_tmp.size() != 0) {
      ...
      if (lb_with_indices(id_vector_b, indices_b, box, result)) {
        ...
        while (id_vector_b[indices_b[result]] == box) {
          ...
          if ((std::find(current_tmp.begin(), current_tmp.end(), new_id) ==
            ...
            // Store it and its subsequent intersections.
            current_tmp.push_back(new_id);
            neighbors.push_back(std::move(neighbors_a[indices_b[result]]));
          }
      ...
      std::move(done_box_ite, done_box_ite + 1, std::back_inserter(current));
...
void discard_ASA_dot_pdt_cm(const Point &cm,
  ...
  if (only_side_ASA == "inside") {
    for (const Finite_cells_iterator &cell_ite : input_cells) {
      ...
      Point test_point = CGAL::centroid(
          cell_ite->vertex(0)->point(), cell_ite->vertex(1)->point(),
          cell_ite->vertex(2)->point(), cell_ite->vertex(3)->point());
      Vector diff_cm = test_point - cm;
      diff_cm =
          diff_cm / (std::sqrt(CGAL::to_double(diff_cm.squared_length())));

      for (const Point &current_point : Calpha_xyz) {
        // if ( current_point == test_point ) continue; // only when checking
        // vtces
        Vector diff_1 = current_point - test_point;
        double diff_norm = std::sqrt(CGAL::to_double(diff_1.squared_length()));
...
void discard_ASA_CACH(const std::vector<Point> &Calpha_xyz,
  ...
  for (const auto &cell_ite : input_cells) {
    ...
    if (!vtx_inside_bool && only_side_ASA == "inside") {
      // Cell has its centroid inside de convex hull determined by the Calphas
      output_cells.push_back(cell_ite);
...
void discard_ASA_dot_pdt_axes(const std::vector<Point> &Calpha_xyz,
  ...
  if (only_side_ASA == "inside") {
    for (const auto &cell_ite : input_cells) {
      ...
      // Now, find Calpha atoms that intersect (w/ a margin of error) these
      // vectors
      for (const auto &CA : Calpha_xyz) {

        // Get difference vector from centroid to current Calpha
        Vector Vdiff = CA - ctd;
        double Vdiff_norm = std::sqrt(CGAL::to_double(Vdiff.squared_length()));
        ...
        // Determine if there is enough of them to classify this cell as being
        // inside
        ...
        if (sum_halves < 4) {
          ...
          if ((!bool_half_xy && !bool_half_yx && bool_half_xz && bool_half_zx &&
               bool_half_yz && bool_half_zy) ||
              (bool_half_xy && bool_half_yx && !bool_half_xz && !bool_half_zx &&
               bool_half_yz && bool_half_zy) ||
              (bool_half_xy && bool_half_yx && bool_half_xz && bool_half_zx &&
               !bool_half_yz && !bool_half_zy)) {
            // inside
...
inline void disregard_lone_cells(const NA_Vector &input_cells,
                                 NA_Vector &output_cells,
                                 const unsigned int lone_threshold) {
...
// than criteria
inline int refine_cell_areas(const Finite_cells_iterator cell_iterator,
...
double get_all_voids(const Delaunay &T, NA_Vector &big_cells,
                     const double min_vol_radius,
                     const double max_area_radius) {
...
inline double cell_volume(const Finite_cells_iterator &cell_iterator) {
  return CGAL::to_double(CGAL::volume(
      cell_iterator->vertex(0)->point(), cell_iterator->vertex(1)->point(),
      cell_iterator->vertex(2)->point(), cell_iterator->vertex(3)->point()));
}
...
double sphere_sector_vol(const Point &p_0, const Point &p_1, const Point &p_2,
  ...
  vec_1 = vec_1 / (std::sqrt(CGAL::to_double(vec_1.squared_length()))) * radius;
  Point point_1 = p_0 + vec_1;
  // get 2nd point of the mini tetrahedron
  Vector vec_2 = p_2 - p_0;
  vec_2 = vec_2 / (std::sqrt(CGAL::to_double(vec_2.squared_length()))) * radius;
  Point point_2 = p_0 + vec_2;
  // get 3rd point of the mini tetrahedron
  Vector vec_3 = p_3 - p_0;
  vec_3 = vec_3 / (std::sqrt(CGAL::to_double(vec_3.squared_length()))) * radius;
  ...
  if (isnan(volume)) {
    volume = 0;
//    std::cerr << "Warning: sphere_sector_vol gave NaN value. Void calculation "
//                 "keeps going."
//              << '\n';
  }
...
inline void discard_CH_0(const NA_Vector &in_cells,
  ...
  for (const auto &cell_ite : in_cells) {
    for (unsigned int i = 0; i <= 3; ++i) {
      ...
      if (!vtx_inside_bool) {
        // Cell has at least 1 vertex inside included area. Keep it and go on
    ...
    case 3: {
      for (unsigned int i = 0; i < 4; i++) {
        if (!intersecting_bool[each][i]) {
          ...
          const double VdW_radius_0 =
              double(in_intersecting_cells[each]->vertex(i)->info().GetRadii());
        ...
        if (!intersecting_bool[each][i]) {
          for (unsigned int j = i + 1; j < 4; j++) {
            if (!intersecting_bool[each][j]) {
              ...
              // Get the points of the vtces of the intersecting cell
              Point p0(in_intersecting_cells[each]->vertex(i)->point());
              Point p1(in_intersecting_cells[each]->vertex(j)->point());
              // p0 p1 are the points inside the included area
              const double VdW_radius_0 = double(
                  in_intersecting_cells[each]->vertex(i)->info().GetRadii());
              const double VdW_radius_1 = double(
                  in_intersecting_cells[each]->vertex(j)->info().GetRadii());
              std::vector<unsigned int> query_vec = {i, j};
              Point p2(in_intersecting_cells[each]
              ...
              Point p3(in_intersecting_cells[each]
                           ...
                           ->point());
              // Store the VdW radii
              std::array<double, 3> temp = {VdW_radius_0, VdW_radius_1, 0};
              in_vtces_radii.push_back(temp);
            ...
            if (!intersecting_bool[each][j]) {
              for (unsigned int k = j + 1; k < 4; k++) {
                if (!intersecting_bool[each][k]) {
                  ...
                  Point p0(in_intersecting_cells[each]->vertex(i)->point());
                  Point p1(in_intersecting_cells[each]->vertex(j)->point());
                  Point p2(in_intersecting_cells[each]->vertex(k)->point());
                  // p0 p1 p2 are the points inside the included area
                  const double VdW_radius_0 = double(in_intersecting_cells[each]
                                                         ->vertex(i)
                                                         ->info()
                                                         .GetRadii());
                  const double VdW_radius_1 = double(in_intersecting_cells[each]
                                                         ->vertex(j)
                                                         ->info()
                                                         .GetRadii());
                  const double VdW_radius_2 = double(in_intersecting_cells[each]
                                                         ->vertex(k)
                                                         ->info()
                                                         .GetRadii());
                  std::vector<unsigned int> query_vec = {i, j, k};
                  Point p3(in_intersecting_cells[each]
                               ...
                               ->point());
                  // Store the VdW radii
                  std::array<double, 3> temp = {VdW_radius_0, VdW_radius_1,
                                                VdW_radius_2};
                  in_vtces_radii.push_back(temp);
...
void na_vector_into_ndd_vector(const NA_Vector &in_cells,
  ...
  for (const auto &each : in_cells) {
    std::array<std::pair<Point, double>, 4> cell = {
        std::make_pair(each->vertex(0)->point(),
                       each->vertex(0)->info().GetRadii()),
        std::make_pair(each->vertex(1)->point(),
                       each->vertex(1)->info().GetRadii()),
        std::make_pair(each->vertex(2)->point(),
                       each->vertex(2)->info().GetRadii()),
        std::make_pair(each->vertex(3)->point(),
                       each->vertex(3)->info().GetRadii())};
...
void tool_PDB_to_CH(const std::string &in_filename,
  ...
  for (const chemfiles::Vector3D &atom_xyz : in_xyz) {
    incl_area_points.push_back(Point(atom_xyz[0], atom_xyz[1], atom_xyz[2]));
  }
  ...
  for (size_t i = 0; i < nsteps; i++) {
    ...
    for (const auto &residuo : input_pdb_top.residues()) {
      ...
      for (const auto &k : residuo) {
        ...
        chemfiles::Atom atomo(input_pdb_top[k].name(), input_pdb_top[k].type());
        atomo.set("is_hetatm",
...
void insert_into_ord_vtor(Vector &v, const T_to_insert &to_insert) {
  typename Vector::iterator i = std::lower_bound(v.begin(), v.end(), to_insert);
...
query_type get_i_not_equal(const std::vector<query_type> &in_vec,
  ...
  if (in_vec.size() < i) {
    throw std::invalid_argument("get_i_not_equal(): specified \"i\" position "
